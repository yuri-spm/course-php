...
$inset = "INSERT INTO users (first_name, last_name, email, document) VALUES ('Yuri', 'Monte', 'yuri.spm@gmail.com', '234589745');";
try{
...
try{
    ...
    $exec = Connect::getInstance()->exec("UPDATE users SET first_name ='Gabriel', last_name = 'Monte' WHERE id ='55'");
    var_dump($exec);
}catch(PDOException $exception){
    var_dump($exception);
}
...
try{
    $exec = Connect::getInstance()->exec("DELETE FROM users WHERE id > 52");
    var_dump($exec);
}catch(PDOException $exception){
    var_dump($exception);
}
